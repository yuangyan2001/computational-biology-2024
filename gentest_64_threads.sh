mkdir -p test_dir_64_thread
cd ./test_dir_64_thread &&
OUTPUT_DIR="64_thread_result/"
mkdir -p $OUTPUT_DIR

for file in "../truncate_160bp/"*
do
    SCRIPT=$(basename ${file})_test_64_thread.sh
    ...
    echo "do" >> $SCRIPT
    echo "perf stat -d -d -d -o ./$OUTPUT_DIR$(basename ${file})_64_thread_res_\$i  blastn -db \$SLURM_TMPDIR/db/nt -query \$SLURM_TMPDIR/query/$(basename ${file}) -out results.out -evalue 1e-5 -num_threads 64" >> $SCRIPT
